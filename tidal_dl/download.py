...
def __setMetaData__(track: Track, album: Album, filepath, contributors, lyrics):
    ...
    obj.album = track.playlist.title
    obj.title = track.title
    if not aigpy.string.isNull(track.version):
        obj.title += ' (' + track.version + ')'

    obj.artist = str(track.artist.name) if track.artist is not None else ""
    obj.copyright = track.copyRight
    obj.tracknumber = track.trackNumberOnPlaylist
    obj.discnumber = "1"
    obj.composer = __parseContributors__('Composer', contributors)
    obj.isrc = track.isrc

    obj.albumartist = "Playlists"
    obj.date = album.releaseDate
    obj.totaldisc = 1
    obj.lyrics = lyrics
    obj.totaltrack = track.playlist.numberOfTracks
